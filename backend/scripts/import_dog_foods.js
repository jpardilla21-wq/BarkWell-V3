...
async function importDogFoods() {
  ...
  try {
    ...
    // Read the JSON file
    const jsonPath = path.join(__dirname, '../../top_100_dog_food_brands_nutritional_data.json');
    ...
    for (const brand of brands) {
      ...
      const ingredients = brand.keyFeatures ? brand.keyFeatures.join(', ') : null;
      ...
      try {
        ...
        if (result.rowCount > 0) {
          insertedCount++;
          console.log(`  ✓ Inserted: ${brandName} - ${productName} (${caloriesPerCup} kcal/cup)`);
        } else {
          skippedCount++;
